...
export type IsRequestParameterType = (
  node: Node,
  type: Type
) => Effect.Effect<never, CompilerPluginError, Option.Option<{name: string, node: Node, type: Type | FakeUnionOrIntersectionType}>>;

export const isRequestParameterService = Context.Tag<IsRequestParameterType>();

const defaultImplementation = isRequestParameterService.of(
  (
    node,
    type
  ) => {
